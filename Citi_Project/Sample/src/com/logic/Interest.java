...
public class Interest 
{
	...
	public double calculateInterest(double base,float bid,float ask)
	{
		...
		if(base>0)
		{
			...
			//System.out.println("IR if base>0"+IR);
		}
		else
		{
			IR=base*ask/365;
	...
	{
		System.out.println("compare:"+IRe+" "+IRg+" "+IRu);
		//IRe=-5;IRg=-2;IRu=-10;
		if(IRe<0 && IRg<0 && IRu<0)
		{
			...
			if(IRe>=IRg)
			{
				if(IRe>=IRu)
				{
					System.out.println("Borrow in EURO "+Math.abs(IRe));
					ctype='e';
				}
				else
				{
					System.out.println("Borrow in USD "+Math.abs(IRu));
					ctype='u';
				}
			}
			else
			{
				if(IRg>=IRu)
				{
					System.out.println("Borrow in GBP "+Math.abs(IRg));
					ctype='g';
				}
				else
				{
		...
		if(IRe>=IRg)
		{
			if(IRe>=IRu)
			{
				System.out.println("Invest in EURO "+IRe);
				ctype='e';
			}
			else
			{
				System.out.println("Invest in USD "+IRu);
				ctype='u';
			}
		}
		else
		{
			if(IRg>=IRu)
			{
				System.out.println("Invest in GBP "+IRg);
				ctype='g';
			}
			else
			{
